...
- **Strict Compilation**: Always run type checking (`npx tsc --noEmit`) and the build script (`pnpm run build`) before finalizing any dependency changes or package configuration edits.
...
1. Add the package to `package.json` under `dependencies` or `devDependencies` using the appropriate pnpm command (e.g., `pnpm install --save-dev <package>`). Avoid editing `package.json` manually to prevent version conflicts.
2. **Validation**: Run `pnpm install` to update the `pnpm-lock.yaml` and ensure there are no installation or resolution conflicts.
...
1. When modifying build scripts, ensure they align with the project standard using `tsdown` for compiling code to `dist/`. The main script should remain `pnpm run build` which invokes `tsdown`.
2. **Validation**: Run `pnpm run build` and verify that output files are successfully generated in `dist/` without compilation errors.
3. **Dev Verification**: Run `pnpm run dev` to ensure watch mode works correctly with the updated setup.
...
1. Run the entire test suite using `pnpm run test` or run target tests with `npx vitest run tests/<path-to-test>`. This project uses Vitest.
...
- **Actions taken**:
  1. Run `pnpm install --save-dev fast-glob`.
  ...
  4. Run `pnpm run build` to confirm `tsdown` compiles the build.
