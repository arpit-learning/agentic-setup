...
## Critical
- Always run `pnpm run build` after making code changes to verify that compiling via tsdown succeeds.
...
1. **Repository Setup**: Install project dependencies.
   - Action: Run `pnpm install` in the project root.
...
3. **Local Development (Watch Mode)**: Watch and compile code dynamically.
   - Action: Run `pnpm run dev` to start compilation in watch mode.
...
4. **Running Tests**: Run tests with Vitest.
   - Action (Full Suite): Run `pnpm run test` or `npx vitest` to run all tests.
