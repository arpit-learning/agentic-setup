...
export class ParallelTaskDisplay {
  ...
  add(
    name: string,
    options?: { depth?: number; pipelineLabel?: string; pipelineRow?: 0 | 1 },
  ): number {
  ...
  private advanceCard(offset: number): void {
    this.currentCard =
      (this.currentCard + offset + this.waitingCards.length) % this.waitingCards.length;
  ...
  private statusIcon(task: TaskState): { char: string; styled: string } {
    switch (task.status) {
      case 'pending':
        return { char: '○', styled: chalk.dim('○') };
      case 'running': {
        const frame = SPINNER_FRAMES[this.spinnerFrame];
        return { char: frame, styled: chalk.cyan(frame) };
      }
      case 'done':
        return { char: '✓', styled: chalk.green('✓') };
      case 'failed':
        return { char: '✗', styled: chalk.red('✗') };
    }
  }
  ...
  private renderPipelineHeader(): string[] {
    ...
    if (branchTasks.length > 0) {
      ...
      const branchLine =
        indent +
        chalk.dim('↘ ') +
        branchNodes.map((n) => n.styled).join(styledArrow) +
        chalk.dim(' ↗');
  ...
  private draw(initial: boolean): void {
    ...
    const previewFresh =
      this.previewLines.length > 0 && Date.now() - this.previewUpdatedAt < PREVIEW_STALE_MS;
    ...
    if (previewFresh && stdout.isTTY) {
      ...
      const cols = stdout.columns || 80;
      if (
        this.currentCard !== this.cachedCardIndex ||
        cols !== this.cachedCardCols ||
        !this.cachedCardLines
      ) {
