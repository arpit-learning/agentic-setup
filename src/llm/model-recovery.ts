...
/**
 * Filter a raw model list down to models that agentic-setup can meaningfully use.
 */
...
export async function handleModelNotAvailable(
  ...
  if (!process.stdin.isTTY) {
    console.error(
      chalk.red(
        `Model "${failedModel}" is not available. Run \`${displayProductName()} config\` to select a different model.`,
      ),
    );
    return null;
  }
  ...
  if (models.length === 0) {
    console.log(
      chalk.red(
        `  No alternative models found. Run \`${displayProductName()} config\` to configure manually.`,
      ),
    );
    return null;
  }
  ...
  if (isDefaultModel) {
    process.env.AGENTIC_SETUP_MODEL = selected;
  } else {
    process.env.AGENTIC_SETUP_FAST_MODEL = selected;
  }
