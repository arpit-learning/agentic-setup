...
const ERROR_PATTERNS: Array<{ pattern: RegExp; message: string | (() => string) }> = [
  ...
  { pattern: /rate limit|too many requests|429/i, message: 'Rate limit exceeded. Retrying...' },
  {
    ...
    message: () =>
      `Usage limit reached. Run \`${displayProductName()} config\` to switch models (e.g. auto or composer-1.5).`,
  },
  {
  ...
  {
    pattern: /project_id is required|missing project_id/i,
    message: () =>
      `GCP Project ID is required. Run \`${displayProductName()} config\` or set the ANTIGRAVITY_PROJECT_ID environment variable.`,
  },
];
