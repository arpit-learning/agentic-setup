...
function spawnClaude(args: string[]): ChildProcess {
  ...
  // Mark the spawned process (and all descendants) as a agentic-setup subprocess so
  // agentic-setup's own hooks short-circuit instead of re-invoking agentic-setup recursively.
  // See src/lib/subprocess-sentinel.ts.
  const env = withAgenticSetupSubprocessEnv(cleanClaudeEnv());
  return IS_WINDOWS
    ? // Windows path: shell:true skips Node's exe quoting, so paths like
      // `C:\Program Files\agentic-setup\claude.cmd` (with spaces) would be parsed by
...
export class ClaudeCliProvider implements LLMProvider {
  ...
  constructor(config: LLMConfig) {
    ...
    const envTimeout = process.env.AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS;
  ...
  async stream(options: LLMStreamOptions, callbacks: LLMStreamCallbacks): Promise<void> {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        ...
        callbacks.onError(
          new Error(
            `Claude CLI timed out after ${this.timeoutMs / 1000}s. Set AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS to increase.`,
          ),
        );
      }
    }, this.timeoutMs);
...
export function isClaudeCliLoggedIn(): boolean {
  ...
  try {
    const result = execFileSync(resolveClaudeBin(), ['auth', 'status'], {
      ...
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 5000,
      env: withAgenticSetupSubprocessEnv(cleanClaudeEnv()),
    });
