...
function runCommand(args: string[], input: string, timeoutMs: number): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `OpenCode timed out after ${timeoutMs / 1000}s. Set AGENTIC_SETUP_OPENCODE_TIMEOUT_MS to increase.`,
        ),
      );
    }, timeoutMs);
...
export class OpenCodeProvider implements LLMProvider {
  ...
  constructor(config: LLMConfig) {
    ...
    const envTimeout = process.env.AGENTIC_SETUP_OPENCODE_TIMEOUT_MS;
