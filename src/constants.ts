...
export const GITHUB_REPO_URL = 'https://github.com/arpit-pm1/agentic-setup';
...
export const BACKUPS_DIR = path.join(AGENTIC_DIR, 'backups');
...
export function getLearningDir(): string {
  ...
  try {
    ...
    _learningDirCache = path.join(mainRoot, AGENTIC_DIR, 'learning');
  } catch {
    _learningDirCache = path.join(AGENTIC_DIR, 'learning');
  }
...
export const REFRESH_LAST_ERROR_FILE = path.join(AGENTIC_DIR, 'last-refresh-error.json');
