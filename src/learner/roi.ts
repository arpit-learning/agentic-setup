...
export function readROIStats(): ROIStats {
  ...
  try {
    ...
    try {
      ...
      console.error(`agentic-setup: roi-stats.json was corrupt — renamed to ${corruptPath}`);
    } catch {
...
function recalculateTotals(stats: ROIStats): void {
  ...
  if (
    ...
    if (rateWithout > 0 && rateWith < rateWithout) {
      ...
      const totalSessions =
        totals.totalSessionsWithLearnings + totals.totalSessionsWithoutLearnings;
      const avgWastePerSession = totalSessions > 0 ? totals.totalWasteTokens / totalSessions : 0;
      totals.estimatedSavingsTokens = Math.round(
        reduction * avgWastePerSession * totals.totalSessionsWithLearnings,
      );
      const avgWasteSecondsPerSession =
        totalSessions > 0 ? totals.totalWasteSeconds / totalSessions : 0;
      totals.estimatedSavingsSeconds = Math.round(
        reduction * avgWasteSecondsPerSession * totals.totalSessionsWithLearnings,
      );
    } else {
...
export function recordSession(
  ...
  if (learnings?.length) {
    for (const entry of learnings) {
      const existingIdx = stats.learnings.findIndex((e) =>
        isSimilarLearning(e.summary, entry.summary),
      );
      if (existingIdx !== -1) {
        stats.learnings[existingIdx].occurrences =
          (stats.learnings[existingIdx].occurrences || 1) + 1;
...
export function formatROISummary(stats: ROIStats): string {
  ...
  if (t.totalSessionsWithoutLearnings > 0) {
    const rateWithout = (t.totalFailuresWithoutLearnings / t.totalSessionsWithoutLearnings).toFixed(
      1,
    );
  ...
  if (t.estimatedSavingsTokens > 0) {
    lines.push(
      `  Estimated savings:             ~${t.estimatedSavingsTokens.toLocaleString()} tokens`,
    );
  }

  if (t.estimatedSavingsSeconds > 0) {
    lines.push(
      `  Time saved:                    at least ${formatDuration(t.estimatedSavingsSeconds)} (not counting human frustration)`,
    );
  } else if (t.totalWasteSeconds > 0) {
    lines.push(
      `  Time wasted on failures:       ${formatDuration(t.totalWasteSeconds)} (and counting)`,
    );
  }
