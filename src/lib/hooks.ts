...
import { resolveCliBinary, isCliCommand, isNpxResolution, pickExecutable } from './resolve-cli.js';
import { bashPath } from '../utils/windows.js';

const SETTINGS_PATH = path.join('.claude', 'settings.json');
const REFRESH_TAIL = 'refresh --quiet';
const HOOK_DESCRIPTION = 'agentic-setup: auto-refreshing docs based on code changes';

function getHookCommand(): string {
  return `${resolveCliBinary()} ${REFRESH_TAIL}`;
}
...
function findHookIndex(sessionEnd: HookMatcher[]): number {
  return sessionEnd.findIndex((entry) =>
    entry.hooks?.some((h) => isCliCommand(h.command, REFRESH_TAIL)),
  );
}
...
const stopHook = createScriptHook({
  eventName: 'Stop',
  scriptPath: path.posix.join('.claude', 'hooks', 'agentic-check-sync.sh'),
  scriptContent: STOP_HOOK_SCRIPT_CONTENT,
  description: 'agentic-setup: offer setup if not configured',
});
...
if [ "$COMMITS_BEHIND" -gt 15 ]; then
  printf '{"systemMessage":"agentic-setup: agent configs are %s commits behind. Run ${bin} refresh to sync."}' "$COMMITS_BEHIND"
fi
`;
}
...
const sessionStartHook = createScriptHook({
  ...
  scriptPath: path.posix.join('.claude', 'hooks', 'agentic-session-freshness.sh'),
  scriptContent: getFreshnessScript,
  description: 'agentic-setup: check config freshness on session start',
});
...
const notificationHook = createScriptHook({
  ...
  scriptPath: path.posix.join('.claude', 'hooks', 'agentic-freshness-notify.sh'),
  scriptContent: getFreshnessScript,
  description: 'agentic-setup: warn when agent configs are stale',
});
...
const PRECOMMIT_ANY_VERSION_BLOCK_RE =
  /\n?#\s*agentic:pre-commit:(?:[a-zA-Z0-9_.-]+:)?start[\s\S]*?#\s*agentic:pre-commit:(?:[a-zA-Z0-9_.-]+:)?end\n?/g;

/**
 * On Windows, when agentic-setup resolves to a `.cmd` shim (npm's default global
...
function tryWindowsDirectNodeInvocation(cmd: string): string | null {
  ...
  // The .cmd shim sits next to a node_modules/agentic-setup/dist/bin.js
  ...
  const binJs = path.join(npmDir, 'node_modules', 'agentic-setup', 'dist', 'bin.js');
...
function getPrecommitBlock(): string {
  ...
  if (npx) {
    // cmd is either 'npx --yes agentic-setup' (bare) or '<npx_path> --yes agentic-setup'.
    ...
    if (path.isAbsolute(npxBinRaw)) {
      ...
      guard = `[ -x "${npxBin}" ]`;
      const npxArgs = cmd.slice(npxBinRaw.length); // ' --yes agentic-setup'
    ...
    if (directNode) {
      ...
      // Fallback: cmd is an absolute path (e.g. /opt/homebrew/bin/agentic-setup,
      // C:\Users\...\agentic-setup.cmd from a pnpm/custom-prefix layout) or bare
      // 'agentic-setup' as last resort. bashPath() converts \\ → / so bash
...
/** True when ANY agentic-setup pre-commit block is present (any version, including legacy unversioned). */
