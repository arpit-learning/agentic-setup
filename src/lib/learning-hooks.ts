...
import { resolveCliBinary, isCliCommand } from './resolve-cli.js';
...
const HOOK_TAILS = [
  {
    event: 'PostToolUse',
    tail: 'learn observe',
    description: 'agentic-setup: recording tool usage for session learning',
  },
  {
    event: 'PostToolUseFailure',
    tail: 'learn observe --failure',
    description: 'agentic-setup: recording tool failure for session learning',
  },
  {
    event: 'UserPromptSubmit',
    tail: 'learn observe --prompt',
    description: 'agentic-setup: recording user prompt for correction detection',
  },
  {
    event: 'SessionEnd',
    tail: 'learn finalize --auto',
    description: 'agentic-setup: finalizing session learnings',
  },
] as const;
...
function hasLearningHook(matchers: HookMatcher[], tail: string): boolean {
  return matchers.some((entry) => entry.hooks?.some((h) => isCliCommand(h.command, tail)));
}
...
function hasCursorHook(entries: Array<{ command: string }>, tail: string): boolean {
  return entries.some((e) => isCliCommand(e.command, tail));
}
...
export function removeCursorLearningHooks(): { removed: boolean; notFound: boolean } {
  ...
  for (const cfg of CURSOR_HOOK_EVENTS) {
    ...
    const idx = entries.findIndex((e) => isCliCommand(e.command, cfg.tail));
    if (idx !== -1) {
...
export function removeLearningHooks(): { removed: boolean; notFound: boolean } {
  ...
  for (const cfg of HOOK_TAILS) {
    ...
    const idx = matchers.findIndex((entry) =>
      entry.hooks?.some((h) => isCliCommand(h.command, cfg.tail)),
    );
    if (idx !== -1) {
