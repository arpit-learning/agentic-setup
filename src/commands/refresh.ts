...
import { AGENTIC_DIR, REFRESH_LAST_ERROR_FILE } from '../constants.js';
...
function writeRefreshError(error: unknown): void {
  try {
    if (!fs.existsSync(AGENTIC_DIR)) fs.mkdirSync(AGENTIC_DIR, { recursive: true });
    fs.writeFileSync(
...
export async function refreshCommand(options: RefreshOptions) {
  ...
  // Skip entirely when running inside a agentic-spawned headless claude session.
  // agentic-setup sets AGENTIC_SETUP_SUBPROCESS=1 in spawnClaude(); hooks inherit it. Without
  ...
  if (quiet && isAgenticSetupSubprocess()) return;
  ...
  // every interactive claude -p the user runs in a agentic-equipped repo.
  // The user can still run `agentic-setup refresh` (no --quiet) for an explicit refresh.
  if (quiet && isHookCascadeFromUserClaudeSession()) return;

  // Skip if another agentic-setup process is already running (e.g. hook fired mid-session)
  if (quiet) {
    const { isAgenticSetupRunning } = await import('../lib/lock.js');
    if (isAgenticSetupRunning()) return;
  }
  ...
  if (!quiet) {
    ...
    if (lastError) {
      ...
      console.log(
        chalk.dim(`     Run with --debug for full details, or report at ${GITHUB_ISSUES_URL}\n`),
      );
      clearRefreshError();
    }
  }
    ...
    if (!config) {
      ...
      console.log(
        chalk.red('No LLM provider configured. Run ') +
          chalk.hex('#83D1EB')(`${displayProductName()} config`) +
