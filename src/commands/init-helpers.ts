...
export function isFirstRun(dir: string): boolean {
  const agenticDir = path.join(dir, '.agentic-setup');
  try {
    const stat = fs.statSync(agenticDir);
...
export function writeErrorLog(
  ...
  try {
    const logPath = path.join(process.cwd(), '.agentic-setup', 'error-log.md');
    const lines = [
    ...
    if (stopReason?.startsWith('timeout')) {
      ...
      lines.push('export AGENTIC_SETUP_STREAM_INACTIVITY_TIMEOUT_MS=180000');
      lines.push('');
      lines.push('# Increase total generation timeout (default: 600s)');
      lines.push('export AGENTIC_SETUP_GENERATION_TIMEOUT_MS=900000');
      lines.push('```');
      lines.push('', 'If timeouts persist, try a different model.');
    }

    fs.mkdirSync(path.join(process.cwd(), '.agentic-setup'), { recursive: true });
    fs.writeFileSync(logPath, lines.join('\n'));
    console.log(chalk.dim(`\n  Error log written to .agentic-setup/error-log.md`));
  } catch {
    // best effort
  }
}
