...
function removeAgenticSetupManagedFiles(dir: string, extension: string): string[] {
  const removed: string[] = [];
  if (!fs.existsSync(dir)) return removed;
  for (const file of fs.readdirSync(dir)) {
    if (file.startsWith(AGENTIC_MANAGED_PREFIX) && file.endsWith(extension)) {
...
export async function uninstallCommand(options: UninstallOptions) {
  ...
  console.log(chalk.dim('    • Cursor rules (agentic-*.mdc)'));
  console.log(chalk.dim('    • AGENTIC_LEARNINGS.md'));
  ...
  const removedCursorRules = removeAgenticSetupManagedFiles(CURSOR_RULES_DIR, '.mdc');
  for (const rule of removedCursorRules) {
    console.log(`  ${chalk.red('✗')} ${rule}`);
  }
  if (removedCursorRules.length > 0) actions.push('cursor rules');

  const removedClaudeRules = removeAgenticSetupManagedFiles(CLAUDE_RULES_DIR, '.md');
  for (const rule of removedClaudeRules) {
    console.log(`  ${chalk.red('✗')} ${rule}`);
  }
  if (removedClaudeRules.length > 0) actions.push('claude rules');

  if (fs.existsSync('AGENTIC_LEARNINGS.md')) {
    fs.unlinkSync('AGENTIC_LEARNINGS.md');
    console.log(`  ${chalk.red('✗')} AGENTIC_LEARNINGS.md`);
    actions.push('learnings file');
  }

  if (removeDirectory(AGENTIC_DIR)) {
    console.log(`  ${chalk.red('✗')} .agentic-setup/ directory`);
    actions.push('.agentic-setup directory');
  }

  if (actions.length === 0) {
    console.log(
      chalk.dim('  Nothing to remove — agentic-setup is not installed in this project.\n'),
    );
    return;
  }
