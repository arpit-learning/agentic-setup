...
export function loadSourcesConfig(dir: string): SourceConfig[] {
  const configPath = path.join(dir, '.agentic-setup', 'sources.json');
  ...
  try {
    ...
    if (!Array.isArray(parsed.sources)) {
      console.warn(
        'Warning: .agentic-setup/sources.json is malformed (missing sources array), skipping sources',
      );
      return [];
    }
    return parsed.sources.filter((s) => s.type && (s.path || s.url));
  } catch {
    console.warn('Warning: .agentic-setup/sources.json is malformed, skipping sources');
    return [];
  }
}

export function writeSourcesConfig(dir: string, sources: SourceConfig[]): void {
  const configDir = path.join(dir, '.agentic-setup');
