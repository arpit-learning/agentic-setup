...
function loadAgenticSetupIgnorePatterns(dir: string, read: typeof fs.readFileSync): string[] {
  try {
    const content = read(path.join(dir, '.agentic-setupignore'), 'utf-8') as string;
    return content
      .split('\n')
      .map((line) => line.trim())
...
function matchesAgenticSetupIgnore(relativePath: string, patterns: string[]): boolean {
...
export function filterIgnoredWarnings(
  ...
  const agenticIgnorePatterns = loadAgenticSetupIgnorePatterns(dir, read);

  return warnings.filter((_, i) => {
    const rel = relativePaths[i];
    if (gitIgnored && gitIgnored.has(rel)) return false;
    if (agenticIgnorePatterns.length > 0 && matchesAgenticSetupIgnore(rel, agenticIgnorePatterns))
      return false;
    return true;
  });
}
