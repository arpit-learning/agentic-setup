...
export function checkQuality(dir: string): Check[] {
  ...
  // 7. AGENTS.md community spec sections
  if (agentsMd) {
    const lower = agentsMd.toLowerCase();
    const hasSetup = /##\s+(setup|getting started|development)/i.test(agentsMd);
    const hasTest = /##\s+(test|testing)/i.test(agentsMd) || lower.includes('npm test');
    const hasConventions =
      /##\s+(convention|coding|style|architecture)/i.test(agentsMd) || lower.includes('convention');
    const specScore = (hasSetup ? 1 : 0) + (hasTest ? 1 : 0) + (hasConventions ? 1 : 0);
    checks.push({
      id: 'agents_md_spec',
      name: 'AGENTS.md spec sections',
      category: 'quality',
      maxPoints: 3,
      earnedPoints: specScore,
      passed: specScore >= 2,
      detail: `setup:${hasSetup} test:${hasTest} conventions:${hasConventions}`,
      suggestion:
        specScore < 2
          ? 'AGENTS.md should include Setup, Testing, and Conventions sections per agents.md guidance'
          : undefined,
    });
  }
