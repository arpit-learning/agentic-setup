...
function detectGitDrift(dir: string): {
  ...
  // (e.g., just written by agentic-setup init but not yet committed)
  try {
  ...
  checks.push({
    ...
    suggestion:
      drift.commitsSinceConfigUpdate > 15
        ? `Code has had ${drift.commitsSinceConfigUpdate} commits since last config update — run \`${displayProductName()} refresh\` to sync`
        : undefined,
    fix:
      drift.commitsSinceConfigUpdate > 15
        ? {
