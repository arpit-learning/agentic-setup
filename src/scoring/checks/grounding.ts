...
export function checkGrounding(dir: string): Check[] {
  ...
  const allProjectEntries = [...projectStructure.dirs, ...projectStructure.files];
  ...
  const groundingRatio =
    meaningfulEntries.length > 0 ? mentioned.length / meaningfulEntries.length : 0;
  ...
  const density =
    mdStructure.nonEmptyLines > 0 ? (totalSpecificRefs / mdStructure.nonEmptyLines) * 100 : 0;
