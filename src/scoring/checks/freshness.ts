 ...
 * (e.g., just written by agentic-setup init but not yet committed), treats it as fresh.
 */
function getCommitsSinceConfigUpdate(dir: string): number | null {
  ...
  // (indicates agentic-setup init just wrote it but hasn't committed yet)
  try {
  ...
  checks.push({
    ...
    suggestion:
      commitsSince !== null && freshnessPoints < 3
        ? `Config is ${commitsSince} commits behind — run \`${displayProductName()} refresh\` to update it`
        : undefined,
    fix:
      commitsSince !== null && freshnessPoints < 3
        ? {
