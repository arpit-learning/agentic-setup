...
export function displayScoreSummary(result: ScoreResult): void {
  ...
  if (failing.length > 0) {
    ...
    console.log(
      chalk.dim(
        `\n  Run ${chalk.hex('#83D1EB')(`${displayProductName()} score`)} for details.${moreText}`,
      ),
    );
  }
  console.log('');
...
export function formatCheckPoints(
  check: Pick<Check, 'earnedPoints' | 'maxPoints'> & {
    earned?: number;
    max?: number;
  },
): string {
...
export function displayScoreOneLiner(result: ScoreResult, drift?: string): void {
  ...
  console.log(
    chalk.gray('  agentic-setup ▸ ') + gc(`${result.score}/100 (${result.grade})`) + driftMsg,
  );
}
