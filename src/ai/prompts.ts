...
2. Undocumented conventions — are there code patterns (commit style, async patterns, error handling) not captured in docs?
3. Stale references — do docs mention removed files, renamed commands, or outdated patterns?`;
...
const FILE_DESCRIPTIONS_RULES = `The "fileDescriptions" object MUST include a one-liner for every file that will be created or modified. Use actual file paths as keys (e.g. "CLAUDE.md", "AGENTS.md", ".cursor/rules/my-rule.mdc", ".claude/rules/api-conventions.md"). Each description should explain why the change is needed, be concise and lowercase.

The "deletions" array should list files that should be removed (e.g. stale configs). Include a reason for each. Omit the array or leave empty if nothing should be deleted.`;
...
- Config drift (7 pts) — handled automatically by agentic-setup (git-based), not your responsibility.
...
AgentSetup schema:
{
  ...
  "claude": {
    "claudeMd": "string (markdown content for CLAUDE.md)",
    "rules": [{ "filename": "string.md", "content": "string (markdown with optional YAML frontmatter)" }]
  },
  "codex": {
    "agentsMd": "string (markdown content for AGENTS.md)"
  },
...
Managed content:
- Keep managed blocks (<!-- agentic:managed --> ... <!-- /agentic:managed -->) intact
- Keep context sync blocks (<!-- agentic:managed:sync --> ... <!-- /agentic:managed:sync -->) intact
- Do NOT modify AGENTIC_LEARNINGS.md — it is managed separately
- Preserve any references to AGENTIC_LEARNINGS.md in CLAUDE.md
...
A markdown section with concise, actionable bullet points. Your output will be written to AGENTIC_LEARNINGS.md — a standalone file that all AI coding agents (Claude Code, Cursor, Codex) reference for project-specific operational patterns.
