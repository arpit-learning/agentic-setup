...
export class StreamParser {
  ...
  getResult(): {
    ...
    let jsonToParse = (this.jsonContent || this.preJsonBuffer).replace(/```\s*$/g, '').trim();
    ...
    if (!this.jsonContent && this.preJsonBuffer) {
      ...
      if (fallbackMatch) {
        ...
        jsonToParse = this.preJsonBuffer
          .slice(matchIndex)
          .replace(/```\s*$/g, '')
          .trim();
      }
    ...
    const explainMatch = this.preJsonBuffer.match(/EXPLAIN:\s*\n([\s\S]*?)(?=\n\s*(`{3}|\{))/);
    if (explainMatch) {
