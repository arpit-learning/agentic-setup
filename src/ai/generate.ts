...
// Large-repo prompts can exceed the 2min first-token window on Vertex.
// AGENTIC_SETUP_STREAM_INACTIVITY_TIMEOUT_MS overrides when users need to push higher.
...
export function buildDiagnostic(
  ...
  if (stopReason === 'timeout_inactivity') {
    ...
    if (charsReceived === 0) {
      return `Model produced no output for ${timeoutSec}s. Check your API key and model name, or increase timeout with AGENTIC_SETUP_STREAM_INACTIVITY_TIMEOUT_MS.`;
    }
    return `Model stopped responding for ${timeoutSec}s after producing ${charsReceived} chars. The model may be stuck. Try a different model or increase timeout with AGENTIC_SETUP_STREAM_INACTIVITY_TIMEOUT_MS.`;
  }

  if (stopReason === 'timeout_total') {
    const timeoutSec = Math.round(
      parseEnvTimeout('AGENTIC_SETUP_GENERATION_TIMEOUT_MS', DEFAULT_TOTAL_TIMEOUT_MS, 5000) / 1000,
    );
    return `Generation exceeded ${timeoutSec}s total time limit. Try a different model or increase timeout with AGENTIC_SETUP_GENERATION_TIMEOUT_MS.`;
  }
...
export async function generateSetup(
  ...
  options?: { skipSkills?: boolean; forceTargetedFix?: boolean; profileHint?: string },
): Promise<{
        ...
        if (config.callbacks && !softWarnFired && SOFT_INACTIVITY_WARN_MS < inactivityTimeoutMs) {
          softWarnTimer = setTimeout(() => {
            ...
            config.callbacks?.onStatus(
              'Model is taking longer than usual on this prompt — large repos may need more time. Set AGENTIC_SETUP_STREAM_INACTIVITY_TIMEOUT_MS to override.',
            );
...
- Every path or name you reference MUST exist in the project — use the file tree provided below.`);
  ...
  if (profileHint) {
    parts.push(`\nProfile guidance: ${profileHint}`);
    parts.push('Reference @./run.md for local startup and health-check commands when present.');
  }
