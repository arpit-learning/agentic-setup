...
${pe.prompt_content}`;
      ...
      const responseStr =
        typeof te.tool_response === 'object' && '_truncated' in te.tool_response
...
function parseAnalysisResponse(raw: string): AnalysisResult {
  ...
  if (!json) {
    return {
      ...
      skills: null,
      explanations: ['LLM response could not be parsed.'],
    };
  }
  ...
  try {
    ...
    return {
      claudeMdLearnedSection: null,
      skills: null,
      explanations: ['LLM response contained invalid JSON.'],
    };
  }
}
...
export async function analyzeEvents(
  ...
  if (existingClaudeMd) {
    contextParts.push(
      `## Existing CLAUDE.md (do NOT repeat these instructions)\n\n${existingClaudeMd.slice(0, 5000)}`,
    );
  }

  if (existingLearnedSection) {
    contextParts.push(
      `## Existing Learned Section (keep these, add new ones, deduplicate)\n\n${existingLearnedSection}`,
    );
  }

  if (existingSkills?.length) {
    const skillsSummary = existingSkills
      .map((s) => `- ${s.filename}: ${s.content.slice(0, 200)}`)
      .join('\n');
  ...
  if (personalLearnings) {
    contextParts.push(
      `## Personal Learnings (developer-specific, do NOT duplicate these)\n\n${personalLearnings}`,
    );
  }
