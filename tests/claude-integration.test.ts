...
import { writeClaudeConfig } from '../src/writers/claude/index.js';
import { getFilesToWrite } from '../src/writers/index.js';
import { computeLocalScore } from '../src/scoring/index.js';
import { readExistingConfigs, AGENTIC_MANAGED_PREFIX } from '../src/fingerprint/existing-config.js';
import { buildGeneratePrompt } from '../src/ai/generate.js';
...
describe('Claude rules: init to disk', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-claude-int-'));
    origCwd = process.cwd();
    process.chdir(dir);
  });
...
describe('Claude rules: scoring', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-score-rules-'));
  });
  ...
  it('excludes agentic-managed rules from scoring', () => {
    mkdirSync(join(dir, '.claude', 'rules'), { recursive: true });
    writeFileSync(
      join(dir, '.claude', 'rules', `${AGENTIC_MANAGED_PREFIX}onboarding.md`),
      '# Managed',
    );
...
describe('Existing config detection', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-config-detect-'));
  });
...
describe('Hook script correctness', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-hooks-int-'));
    origCwd = process.cwd();
    process.chdir(dir);
  });
  ...
  it('SessionStart hook references correct state file path', async () => {
    ...
    const scriptPath = join(dir, '.claude', 'hooks', 'agentic-session-freshness.sh');
    expect(existsSync(scriptPath)).toBe(true);

    const script = readFileSync(scriptPath, 'utf-8');
    expect(script).toContain('.agentic-setup/.agentic-state.json');
    expect(script).not.toContain('".agentic-setup/state.json"');
  });
  ...
  it('SessionStart hook outputs systemMessage (not notification)', async () => {
    ...
    const script = readFileSync(
      join(dir, '.claude', 'hooks', 'agentic-session-freshness.sh'),
      'utf-8',
    );
  ...
  it('Stop hook outputs decision:block', async () => {
    ...
    const script = readFileSync(join(dir, '.claude', 'hooks', 'agentic-check-sync.sh'), 'utf-8');
  ...
  it('createScriptHook remove cleans up script and settings', async () => {
    const { installSessionStartHook, removeSessionStartHook } = await import('../src/lib/hooks.js');
...
// ── AGENTIC_MANAGED_PREFIX consistency ─────────────────────────────────

describe('AGENTIC_MANAGED_PREFIX usage', () => {
  it('prefix is agentic-', () => {
    expect(AGENTIC_MANAGED_PREFIX).toBe('agentic-');
  });
});
