...
import { checkBonus } from '../../src/scoring/checks/bonus.js';

describe('checkBonus model_pinned', () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-bonus-model-'));
  });
  ...
  it('passes when CLAUDE.md mentions AGENTIC_SETUP_MODEL', () => {
    writeFileSync(join(dir, 'CLAUDE.md'), 'Use `AGENTIC_SETUP_MODEL` in CI.');
