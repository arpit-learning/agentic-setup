...
import { computeLocalScore, detectTargetAgent } from '../../src/scoring/index.js';

describe('detectTargetAgent', () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-detect-'));
  });
...
describe('computeLocalScore target filtering', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'agentic-score-'));
  });
