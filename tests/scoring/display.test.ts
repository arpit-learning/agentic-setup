...
import type { ScoreResult, Check } from '../../src/scoring/index.js';
...
describe('displayScoreSummary', () => {
  ...
  it('shows agentic-setup score hint only once', () => {
    const result = makeScoreResult({
      score: 50,
      grade: 'C',
    ...
    displayScoreSummary(result);
    const output = logs.join('\n');
    const matches = output.match(/agentic-setup score/g);
  ...
  it('shows no failing section when all checks pass', () => {
    ...
    displayScoreSummary(result);
    const output = logs.join('\n');
    expect(output).not.toContain('agentic-setup score');
