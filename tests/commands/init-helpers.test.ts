...
import os from 'os';
import { isFirstRun, summarizeSetup, derivePermissions } from '../../src/commands/init-helpers.js';
import { detectAgents } from '../../src/commands/init-prompts.js';
import { formatProjectPreview, formatWhatChanged } from '../../src/commands/init-display.js';
import { detectPlatforms } from '../../src/scanner/index.js';
...
describe('isFirstRun', () => {
  ...
  it('returns true when .agentic-setup/ does not exist', () => {
    expect(isFirstRun(tmpDir)).toBe(true);
  });

  it('returns false when .agentic-setup/ directory exists', () => {
    fs.mkdirSync(path.join(tmpDir, '.agentic-setup'));
    expect(isFirstRun(tmpDir)).toBe(false);
  });

  it('returns true when .agentic-setup is a file not a directory', () => {
    fs.writeFileSync(path.join(tmpDir, '.agentic-setup'), 'not a dir');
    expect(isFirstRun(tmpDir)).toBe(true);
  });
});
