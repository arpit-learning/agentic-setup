...
vi.mock('../../src/learner/roi.js', async () => {
  const actual =
    await vi.importActual<typeof import('../../src/learner/roi.js')>('../../src/learner/roi.js');
  return {
    ...actual,
    readROIStats: vi.fn(),
  };
...
vi.mock('../../src/lib/learning-hooks.js', () => ({
...
import { readROIStats } from '../../src/learner/roi.js';
import { areLearningHooksInstalled } from '../../src/lib/learning-hooks.js';
import type { ROIStats, ROITotals } from '../../src/learner/roi.js';
...
describe('insights command', () => {
  ...
  it('shows install message when no hooks and no data', async () => {
    ...
    expect(output).toContain('agentic-setup learn install');
  });
