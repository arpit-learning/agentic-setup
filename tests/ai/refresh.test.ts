...
describe('refreshDocs', () => {
  ...
  it('filters agentic-managed rules from prompt', async () => {
    const prompt = await getRefreshPrompt({
      claudeRules: [{ filename: 'agentic-onboarding.md', content: '# Managed' }],
    });
    expect(prompt).not.toContain('agentic-onboarding.md');
  });
  ...
  it('passes AGENTIC_SETUP_FAST_MODEL as model override when set', async () => {
