...
import { validateSetup, scoreAndRefine } from '../../src/ai/score-refine.js';
import type { ProjectStructure } from '../../src/scoring/utils.js';
...
import { llmCall } from '../../src/llm/index.js';
...
describe('validateSetup', () => {
  ...
  it('detects directory tree listings', () => {
    const treeLines = Array.from({ length: 15 }, (_, i) => `│   ├── file${i}.ts`);
    const setup = makeSetup(['## Structure', '', '```', ...treeLines, '```'].join('\n'));
...
describe('scoreAndRefine', () => {
  ...
  it('includes skills in fix when point-losing issues also exist', async () => {
    ...
    const setup = makeSetupWithSkills('## A\n\n- `src/nonexistent.ts` ref\n\n## B\n\n## C', [
      {
        name: 'test-skill',
        description: 'A skill',
        content: [
    ...
    const history: Array<{ role: 'user' | 'assistant'; content: string }> = [];
    const result = await scoreAndRefine(setup, '/project', history, undefined, { thorough: true });
