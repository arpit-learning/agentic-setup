...
import fs from 'fs';
import { computeFingerprintHash, collectFingerprint, Fingerprint } from '../../src/fingerprint/index.js';

vi.mock('fs');
vi.mock('../../src/fingerprint/git.js', () => ({ getGitRemoteUrl: () => 'https://github.com/test/repo' }));
vi.mock('../../src/fingerprint/file-tree.js', () => ({ getFileTree: () => ['src/index.ts', 'package.json'] }));
vi.mock('../../src/fingerprint/existing-config.js', () => ({ readExistingConfigs: () => ({}) }));
vi.mock('../../src/fingerprint/code-analysis.js', () => ({
  analyzeCode: () => ({
    files: [],
    truncated: false,
...
describe('computeFingerprintHash', () => {
  ...
  it('uses gitRemoteUrl and packageName as key', () => {
    const fp1 = {
      ...baseFingerprint,
      gitRemoteUrl: 'https://github.com/a/b',
      packageName: 'app-a',
    };
    const fp2 = {
      ...baseFingerprint,
  ...
  it('same hash for fingerprints with same url and package but different languages', () => {
    const fp1 = {
      ...baseFingerprint,
      gitRemoteUrl: 'git@github.com:test/repo',
      packageName: 'app',
    };
...
describe('collectFingerprint', () => {
  ...
  it('includes gitRemoteUrl, fileTree, existingConfigs, and codeAnalysis', async () => {
    ...
    expect(fp.codeAnalysis).toEqual({
      files: [],
      truncated: false,
      totalProjectTokens: 0,
      compressedTokens: 0,
