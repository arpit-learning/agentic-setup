...
import os from 'os';
import { loadFingerprintCache, saveFingerprintCache, computeTreeSignature } from '../../src/fingerprint/cache.js';
import type { CodeAnalysis } from '../../src/fingerprint/code-analysis.js';

function makeTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-cache-'));
}
...
describe('fingerprint cache', () => {
  ...
  describe('saveFingerprintCache + loadFingerprintCache', () => {
    ...
    it('returns null when cache file is corrupt JSON', () => {
      const tmp = makeTempDir();
      tmpDirs.push(tmp);
      const cacheDir = path.join(tmp, '.agentic-setup', 'cache');
    ...
    it('creates .agentic-setup/cache directory if missing', () => {
      const tmp = makeTempDir();
      tmpDirs.push(tmp);
      const cachePath = path.join(tmp, '.agentic-setup', 'cache', 'fingerprint.json');
