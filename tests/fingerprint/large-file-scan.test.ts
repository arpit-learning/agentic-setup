/**
 * tests/fingerprint/large-file-scan.test.ts
 *
 * Unit tests for scanLargeFiles().
 *
 * All tests use injected statSync / readdirSync stubs (same pattern as
 * tests/ai/detect.test.ts).  Zero disk I/O, zero temp files.
 *
...
} from '../../src/fingerprint/large-file-scan.js';
...
describe('scanLargeFiles', () => {
  ...
  it('respects a custom ignoreDirs set', () => {
    ...
    expect(scanLargeFiles('/project', { statSync, readdirSync })).toHaveLength(1);
  ...
  it('re-throws unexpected errors from readdirSync', () => {
    ...
    expect(() => scanLargeFiles('/project', { statSync, readdirSync })).toThrow(
