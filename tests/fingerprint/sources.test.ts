...
import {
  ...
  computeSourceHashes,
  type SourceSummary,
} from '../../src/fingerprint/sources.js';

function mkTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-sources-test-'));
}
...
describe('loadSourcesConfig', () => {
  ...
  it('loads valid sources.json', () => {
    fs.mkdirSync(path.join(dir, '.agentic-setup'), { recursive: true });
    fs.writeFileSync(
      path.join(dir, '.agentic-setup', 'sources.json'),
      JSON.stringify({
        sources: [
  ...
  it('returns empty array for malformed JSON', () => {
    fs.mkdirSync(path.join(dir, '.agentic-setup'), { recursive: true });
    fs.writeFileSync(path.join(dir, '.agentic-setup', 'sources.json'), 'not json');
    expect(loadSourcesConfig(dir)).toEqual([]);
  });

  it('returns empty array for missing sources array', () => {
    fs.mkdirSync(path.join(dir, '.agentic-setup'), { recursive: true });
    fs.writeFileSync(path.join(dir, '.agentic-setup', 'sources.json'), '{"foo": 1}');
    expect(loadSourcesConfig(dir)).toEqual([]);
  });
  ...
  it('filters out entries without type or path', () => {
    ...
    fs.writeFileSync(
      path.join(dir, '.agentic-setup', 'sources.json'),
      JSON.stringify({
        sources: [{ type: 'repo', path: '../valid' }, { path: '../no-type' }, { type: 'repo' }],
      }),
    );
...
describe('writeSourcesConfig', () => {
  ...
  it('creates .agentic-setup dir and writes sources.json', () => {
    writeSourcesConfig(dir, [{ type: 'repo', path: '../sibling' }]);
    const content = JSON.parse(
      fs.readFileSync(path.join(dir, '.agentic-setup', 'sources.json'), 'utf-8'),
    );
...
describe('resolveAllSources', () => {
  ...
  it('resolves config sources', () => {
    const relativeSibling = path.relative(projectDir, siblingDir);
    fs.mkdirSync(path.join(projectDir, '.agentic-setup'), { recursive: true });
    fs.writeFileSync(
      path.join(projectDir, '.agentic-setup', 'sources.json'),
