...
const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-roi-test-'));

vi.mock('../../src/constants.js', async () => {
  const actual = await vi.importActual<typeof import('../../src/constants.js')>(
    '../../src/constants.js',
  );
  return {
    ...actual,
...
import { readROIStats, writeROIStats, recordSession, formatROISummary } from '../../src/learner/roi.js';
import type { ROIStats, SessionROISummary, LearningCostEntry } from '../../src/learner/roi.js';
...
describe('ROI stats', () => {
  ...
  it('recordSession with learnings appends both and updates waste total', () => {
    const learnings = [
      ...
      makeLearningEntry({
        wasteTokens: 200,
        observationType: 'fix',
        summary: 'always run npm ci before deploy',
      }),
    ];
  ...
  it('returns zero savings when failure rates are equal', () => {
    for (let i = 0; i < 3; i++) {
      recordSession(
        makeSession({ sessionId: `wo-${i}`, hadLearningsAvailable: false, failureCount: 3 }),
      );
    }
    for (let i = 0; i < 3; i++) {
      recordSession(
        makeSession({ sessionId: `wi-${i}`, hadLearningsAvailable: true, failureCount: 3 }),
      );
    }
  ...
  it('returns zero savings when failure rate is worse with learnings', () => {
    for (let i = 0; i < 3; i++) {
      recordSession(
        makeSession({ sessionId: `wo-${i}`, hadLearningsAvailable: false, failureCount: 1 }),
      );
    }
    for (let i = 0; i < 3; i++) {
      recordSession(
        makeSession({ sessionId: `wi-${i}`, hadLearningsAvailable: true, failureCount: 5 }),
      );
    }
