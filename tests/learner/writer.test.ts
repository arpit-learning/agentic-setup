...
import { writeLearnedContent, readLearnedSection } from '../../src/learner/writer.js';
...
describe('writer', () => {
  ...
  describe('deduplication', () => {
    it('removes duplicate bullets by substring match', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => String(p) === 'AGENTIC_LEARNINGS.md');
      vi.mocked(fs.readFileSync).mockReturnValue(
        '# agentic-setup Learnings\n\nSome header.\n\n- Use pnpm for installs\n',
      );
      ...
      expect(result.written).toContain('AGENTIC_LEARNINGS.md');
    });
  ...
  describe('writeLearnedContent', () => {
    it('creates AGENTIC_LEARNINGS.md with header when no file exists', () => {
      vi.mocked(fs.existsSync).mockReturnValue(false);
      ...
      expect(result.written).toContain('AGENTIC_LEARNINGS.md');
      const written = vi.mocked(fs.writeFileSync).mock.calls[0][1] as string;
      expect(written).toContain('# agentic-setup Learnings');
    });
  ...
  describe('personal learnings routing', () => {
    ...
    it('routes :project bullets to AGENTIC_LEARNINGS.md', () => {
      vi.mocked(fs.existsSync).mockReturnValue(false);
      ...
      const calls = vi.mocked(fs.writeFileSync).mock.calls;
      const projectCall = calls.find((c) => String(c[0]) === 'AGENTIC_LEARNINGS.md');
  ...
  describe('readLearnedSection', () => {
    it('reads bullets from AGENTIC_LEARNINGS.md', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => String(p) === 'AGENTIC_LEARNINGS.md');
      vi.mocked(fs.readFileSync).mockReturnValue(
        '# agentic-setup Learnings\n\nSome description.\n\n- Use pnpm\n- Run tests\n',
      );
    ...
    it('falls back to old inline section in CLAUDE.md without migrating', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        if (String(p) === 'AGENTIC_LEARNINGS.md') return false;
