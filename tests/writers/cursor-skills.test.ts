...
import { writeCursorConfig } from '../../src/writers/cursor/index.js';
...
describe('writeCursorConfig — skills', () => {
  ...
  it('writes skills to .cursor/skills/{name}/SKILL.md with frontmatter', () => {
    ...
    expect(written).toContain(path.join('.cursor', 'rules', 'agentic-pre-commit.mdc'));
  ...
  it('writes pre-commit, learnings, and sync rules even when no skills provided', () => {
    ...
    expect(written).toContain(path.join('.cursor', 'rules', 'agentic-pre-commit.mdc'));
    expect(written).toContain(path.join('.cursor', 'rules', 'agentic-learnings.mdc'));
    expect(written).toContain(path.join('.cursor', 'rules', 'agentic-sync.mdc'));
  });
