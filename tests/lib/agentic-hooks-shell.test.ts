...
const SCRIPT = path.resolve(process.cwd(), '.claude/hooks/agentic-check-sync.sh');

function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-hooks-test-'));
...
describe('agentic-check-sync.sh', () => {
  let tmpDir: string;
  ...
  afterEach(() => {
    ...
    try {
      const files = fs.readdirSync(os.tmpdir()).filter((f) => f.startsWith('agentic-nudge-'));
  ...
  it('exits 0 silently when the project directory is not a git repo', () => {
    ...
    // (scratch dir, model archive, etc.). agentic-setup refresh won't run there
    ...
    const nonGitDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-nogit-'));
    try {
  ...
  it('exits 0 immediately when AGENTIC_SETUP_SUBPROCESS=1 (spawned subprocess)', () => {
    const { status, stdout } = runScript(tmpDir, { AGENTIC_SETUP_SUBPROCESS: '1' });
    expect(status).toBe(0);
    expect(stdout).not.toContain('"decision":"block"');
  });

  it('exits 0 immediately when AGENTIC_SETUP_SPAWNED=1', () => {
    const { status, stdout } = runScript(tmpDir, { AGENTIC_SETUP_SPAWNED: '1' });
    expect(status).toBe(0);
    expect(stdout).not.toContain('"decision":"block"');
  });

  it('exits 0 when agentic-setup is present in the pre-commit hook', () => {
    fs.writeFileSync(
  ...
  it('outputs block decision when agentic-setup is not set up and flag is not set', () => {
  ...
  it('exits 0 when $PWD is a nested git repo but $CLAUDE_PROJECT_DIR is non-git', () => {
    ...
    // for a project agentic-setup has no business managing.
    //
    ...
    const owner = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-owner-'));
  ...
  it('honors $CLAUDE_PROJECT_DIR when set, even when $PWD is unrelated', () => {
    ...
    // whose pre-commit lacks agentic-setup → nudge should fire. Confirms
