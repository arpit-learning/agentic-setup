...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-lock-test-'));
...
function lockPathForCwd(): string {
  ...
  return path.join(os.tmpdir(), `.agentic-setup-${hash}.lock`);
}

describe('isAgenticSetupRunning', () => {
  it('returns false when no lock file exists', async () => {
    const { isAgenticSetupRunning } = await import('../../src/lib/lock.js');
    expect(isAgenticSetupRunning()).toBe(false);
  });
  ...
  it('returns false when lock file contains the current process PID', async () => {
    ...
    // learnFinalizeCommand calls isAgenticSetupRunning() and must NOT treat itself
    // as "another agentic-setup process is running"
  ...
  it('returns true when lock is held by a different live process', async () => {
    ...
    const { isAgenticSetupRunning } = await import('../../src/lib/lock.js');
    expect(isAgenticSetupRunning()).toBe(true);
  });
...
describe('acquireLock / releaseLock', () => {
  ...
  it('releaseLock does not throw when no lock file exists', async () => {
    const { releaseLock } = await import('../../src/lib/lock.js');
