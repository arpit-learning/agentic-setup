...
const CONFIG_DIR = path.join('/home/user', '.agentic-setup');
...
describe('config', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.AGENTIC_SETUP_MODEL;
  ...
  describe('resolveFromEnv', () => {
    ...
    it('respects AGENTIC_SETUP_MODEL override', () => {
      process.env.ANTHROPIC_API_KEY = 'sk-ant-test';
      process.env.AGENTIC_SETUP_MODEL = 'claude-opus-4-6';
      const config = resolveFromEnv();
    ...
    it('returns cursor config when AGENTIC_SETUP_USE_CURSOR_SEAT is set', () => {
    ...
    it('returns cursor config when AGENTIC_SETUP_USE_CURSOR_SEAT is "true"', () => {
      process.env.AGENTIC_SETUP_USE_CURSOR_SEAT = 'true';
      const config = resolveFromEnv();
      expect(config?.provider).toBe('cursor');
    });

    it('respects AGENTIC_SETUP_MODEL for cursor seat', () => {
      process.env.AGENTIC_SETUP_USE_CURSOR_SEAT = '1';
      process.env.AGENTIC_SETUP_MODEL = 'auto';
      const config = resolveFromEnv();
      expect(config).toEqual({
        provider: 'cursor',
        model: 'auto',
    ...
    it('returns claude-cli config when AGENTIC_SETUP_USE_CLAUDE_CLI is set', () => {
      process.env.AGENTIC_SETUP_USE_CLAUDE_CLI = '1';
      const config = resolveFromEnv();
      expect(config).toEqual({
        provider: 'claude-cli',
    ...
    it('returns claude-cli config when AGENTIC_SETUP_USE_CLAUDE_CLI is "true"', () => {
      process.env.AGENTIC_SETUP_USE_CLAUDE_CLI = 'true';
      const config = resolveFromEnv();
      expect(config?.provider).toBe('claude-cli');
    });

    it('respects AGENTIC_SETUP_MODEL for claude-cli seat', () => {
      process.env.AGENTIC_SETUP_USE_CLAUDE_CLI = '1';
      process.env.AGENTIC_SETUP_MODEL = 'claude-sonnet-4-6';
      const config = resolveFromEnv();
      expect(config).toEqual({
        provider: 'claude-cli',
  ...
  describe('getFastModel', () => {
    it('returns AGENTIC_SETUP_FAST_MODEL when set', () => {
      process.env.AGENTIC_SETUP_FAST_MODEL = 'gpt-5.4-mini';
    ...
    it('prefers AGENTIC_SETUP_FAST_MODEL over ANTHROPIC_SMALL_FAST_MODEL', () => {
      process.env.AGENTIC_SETUP_FAST_MODEL = 'gpt-5.4-mini';
      process.env.ANTHROPIC_SMALL_FAST_MODEL = 'claude-haiku-4-5';
      expect(getFastModel()).toBe('gpt-5.4-mini');
    });
    ...
    it('env var overrides config file fastModel and provider default', () => {
      process.env.AGENTIC_SETUP_FAST_MODEL = 'env-fast-model';
      process.env.ANTHROPIC_API_KEY = 'sk-ant-test';
      vi.mocked(fs.existsSync).mockReturnValue(true);
  ...
  describe('getMaxPromptTokens', () => {
    ...
    it('respects AGENTIC_SETUP_MODEL env var override', () => {
      process.env.AGENTIC_SETUP_MODEL = 'gpt-4o';
      process.env.ANTHROPIC_API_KEY = 'test-key';
      vi.mocked(fs.existsSync).mockReturnValue(false);
    ...
    it('falls back to default context window for unknown model', () => {
      process.env.AGENTIC_SETUP_MODEL = 'some-future-model';
      process.env.ANTHROPIC_API_KEY = 'test-key';
      vi.mocked(fs.existsSync).mockReturnValue(false);
