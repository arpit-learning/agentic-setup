...
describe('parseCursorModelList', () => {
  it('parses model list from error output', () => {
    const output =
      'Cannot use this model: __agentic_probe__. Available models: auto, composer-2, claude-4.6-sonnet-medium';
...
describe('listCursorModels', () => {
  ...
  it('parses models from error thrown by execFileSync', async () => {
    const errOutput =
      'Cannot use this model: __agentic_probe__. Available models: auto, composer-2-fast';
    if (process.platform === 'win32') {
      execSync.mockImplementation(() => {
