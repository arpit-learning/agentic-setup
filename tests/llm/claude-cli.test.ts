...
describe('ClaudeCliProvider', () => {
  ...
  it('uses AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS when set', () => {
    const orig = process.env.AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS;
    process.env.AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS = '120000';
    const provider = new ClaudeCliProvider({ provider: 'claude-cli', model: 'default' });
    expect(provider).toBeDefined();
    process.env.AGENTIC_SETUP_CLAUDE_CLI_TIMEOUT_MS = orig;
  });
...
describe('cleanClaudeEnv allowlist behavior (F-P0-1)', () => {
  ...
  async function callClean() {
    const mod = await import('../../src/llm/claude-cli.js');
