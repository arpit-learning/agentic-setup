...
import { trackUsage } from '../../src/llm/usage.js';
...
describe('OpenCodeProvider', () => {
  ...
  it('call() rejects on timeout', async () => {
    ...
    const origTimeout = process.env.AGENTIC_SETUP_OPENCODE_TIMEOUT_MS;
    process.env.AGENTIC_SETUP_OPENCODE_TIMEOUT_MS = '100';
    const provider = new OpenCodeProvider({ provider: 'opencode', model: 'default' });
    const promise = provider.call({ system: 'S', prompt: 'P' });
    ...
    if (origTimeout !== undefined) {
      process.env.AGENTIC_SETUP_OPENCODE_TIMEOUT_MS = origTimeout;
    } else {
      delete process.env.AGENTIC_SETUP_OPENCODE_TIMEOUT_MS;
    }
  });
