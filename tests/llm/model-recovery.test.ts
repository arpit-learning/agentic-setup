...
vi.unmock('../../src/llm/model-recovery.js');
...
describe('handleModelNotAvailable', () => {
  ...
  const originalAgenticSetupModel = process.env.AGENTIC_SETUP_MODEL;
  const originalAgenticSetupFastModel = process.env.AGENTIC_SETUP_FAST_MODEL;
  ...
  afterEach(() => {
    ...
    if (originalAgenticSetupModel !== undefined)
      process.env.AGENTIC_SETUP_MODEL = originalAgenticSetupModel;
    else delete process.env.AGENTIC_SETUP_MODEL;
    if (originalAgenticSetupFastModel !== undefined)
      process.env.AGENTIC_SETUP_FAST_MODEL = originalAgenticSetupFastModel;
    else delete process.env.AGENTIC_SETUP_FAST_MODEL;
  });
  ...
  it('saves default model to config when the failed model is the default', async () => {
    ...
    expect(process.env.AGENTIC_SETUP_MODEL).toBe('claude-haiku-4-5-20251001');
  });
  ...
  it('saves fast model to config when the failed model differs from default', async () => {
    ...
    expect(process.env.AGENTIC_SETUP_FAST_MODEL).toBe('claude-haiku-4-5-20251001');
  });
