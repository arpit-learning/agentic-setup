...
async function runSync(token) {
  const branchPrefix = getInput('sync-branch-prefix') || 'agentic-setup/sync';
  ...
  // Run agentic-setup refresh
  try {
    execFileSync('npx', ['--yes', AGENTIC_PKG, 'refresh', '--quiet'], {
      encoding: 'utf-8',
      timeout: 300000,
      env: { ...process.env, AGENTIC_SETUP_SKIP_UPDATE_CHECK: '1' },
    });
  ...
  // Configure git
  execFileSync('git', ['config', 'user.name', 'agentic-setup[bot]']);
  execFileSync('git', ['config', 'user.email', 'agentic-setup-bot@users.noreply.github.com']);
  ...
  execFileSync('git', ['commit', '-m', `[agentic-setup] sync ${formatNames}`]);
  ...
  if (token) {
    ...
    const body = formats.length > 0
      ? `agentic-setup automatically synced the following agent configs with latest code changes:\n\n${formats.map(f => `- **${f.name}** (\`${f.file}\`)`).join('\n')}\n\nMerge this PR to keep all AI agents up to date with the codebase.`
      : 'agentic-setup refreshed agent configuration files.';

    try {
      const pr = await githubApi('POST', `/repos/${repo}/pulls`, {
        title: `[agentic-setup] Sync agent configs (${date})`,
        body,
        head: syncBranch,
        base: defaultBranch,
      }, token);
...
async function run() {
  ...
  let resultJson;
  try {
    const output = execFileSync('npx', ['--yes', AGENTIC_PKG, 'score', '--json', '--quiet', '--agent', agent], {
      encoding: 'utf-8',
      timeout: 120000,
      env: { ...process.env, AGENTIC_SETUP_SKIP_UPDATE_CHECK: '1' },
    });
    ...
    setFailed(`Failed to run agentic-setup score: ${err.message}`);
    return;
  }
        ...
        if (baseBranch) {
          try {
            ...
            const baseOutput = execFileSync(
              'npx', ['--yes', AGENTIC_PKG, 'score', '--json', '--quiet', '--agent', agent, '--compare', `origin/${baseBranch}`],
              { encoding: 'utf-8', timeout: 120000, env: { ...process.env, AGENTIC_SETUP_SKIP_UPDATE_CHECK: '1' } },
            );
